...
interface MeResponse{data:{user:{id:string;email?:string};merchant:MerchantProfile|null}}
...
async function loadBackendIdentity(user:User):Promise<AuthResult>{const response=await request<MeResponse>('/api/me',{auth:true});return{user:mapUser(user),merchant:response.data.merchant,session:(await supabase.auth.getSession()).data.session}}
async function createMerchant(input:MerchantProfileUpdate):Promise<MerchantProfile>{return (await request<{data:MerchantProfile}>('/api/merchants/me',{method:'PUT',auth:true,body:JSON.stringify({businessName:input.businessName,momoPhone:input.momoPhone})})).data}

export async function signUpMerchant(input:MerchantSignUpInput):Promise<AuthResult>{requireConfig();const {data,error}=await supabase.auth.signUp({email:input.email,password:input.password,options:{data:{full_name:input.ownerName,role:'merchant',phone:input.phone,momo_network:input.momoNetwork}}});if(error)throw error;if(!data.user)throw new Error('Account creation did not return a user.');localStorage.setItem(PENDING_KEY,JSON.stringify({email:input.email.toLowerCase(),businessName:input.businessName,momoPhone:toInternationalPhone(input.momoNumber)}));if(!data.session)return{user:mapUser(data.user),merchant:null,session:null,requiresEmailConfirmation:true};const merchant=await createMerchant({businessName:input.businessName,momoPhone:toInternationalPhone(input.momoNumber)});localStorage.removeItem(PENDING_KEY);return{user:mapUser(data.user),merchant,session:data.session}}
