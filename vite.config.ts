...
export default defineConfig(({mode})=>{
  ...
  return{
    ...
    define:{
      ...
      'import.meta.env.VITE_USDC_CONTRACT_ADDRESS':JSON.stringify(env.VITE_USDC_CONTRACT_ADDRESS||env.USDC_CONTRACT_ADDRESS||'')
    },
    server:{proxy:{'/api':{target:'http://localhost:4000',changeOrigin:true},'/health':{target:'http://localhost:4000',changeOrigin:true}}}
  };
});
