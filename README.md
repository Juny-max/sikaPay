...
SikaPay is an Ethereum-to-Mobile Money payment bridge designed for Ghana. Customers pay an invoice with ETH or USDC; after onchain confirmation, the merchant receives the Ghana Cedi equivalent on Mobile Money. This repository contains the complete hackathon frontend.
...
The application runs at `http://localhost:5173`. For a production check, run `pnpm build` and `pnpm preview`.
